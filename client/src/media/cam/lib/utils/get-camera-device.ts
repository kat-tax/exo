import type {CameraDevice, CameraPosition, DeviceFilter} from 'react-native-vision-camera';

/**
 * Get the best matching Camera device that best satisfies your requirements using a sorting filter
 * Polyfill for react-native-vision-camera's getCameraDevice function
 *
 * @param devices Array of available camera devices
 ...
 * @returns The Camera device that matches your filter the closest, or `undefined` if no such Camera Device exists on the given position
 *
 * @example
 * ```ts
